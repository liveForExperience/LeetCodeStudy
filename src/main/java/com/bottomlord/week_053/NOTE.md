...
# LeetCode_286_墙与门
## 题目
你被给定一个m × n的二维网格，网格中有以下三种可能的初始化值：
```
-1表示墙或是障碍物
0表示一扇门
...
### 思路
动态规划：
- 从公主所在的右下角反着找起始点，否则从起始点开始找，就需要确定两种状态
    - 上一步到当前这一步的最小hp
    - 之后的所有可能路径的最小hp
- 而从公主处反着找，只需要确定路径需要hp的最小值就可以一路找到起始点
- `dp[i][j]`：坐标(i,j)到达终点所需的最小hp，hp至少为1，否则会判定为死亡
- 状态转移方程：
    - `dp[i][j] = Math.max(Math.min(dp[i + 1][j], dp[i][j + 1]) - dungeon[i][j], 1)`
    - 首先确定向右和向下的两个路径中所需的最小hp值
    - 状态转移到当前坐标，就是用最小hp - 当前所需的hp：
        - 如果当前位置是恢复hp，那么就代表之后要的hp可以从当前恢复的里面取
        - 如果当前恢复的hp大于之后需要的hp，那么当前位置只需要是最小维持生命的1就可以了
        - 如果是扣的，那么就代表之后需要的hp需要加上当前扣除的值才是当前位置需要的hp
- 初始化：所有越界位置都应该是int的最大值，这样在判断路径时，越界的值会自动忽略，而起始的终点位置，其dp值应该是1和终点状态之间的关系，所以`dp[row - 1][col]`和`dp[row][col - 1]`是1
- 返回`dp[0][0]`
### 代码
```java
class Solution {
