...
## 解法
### 思路
- 计算每个节点的过载量，过载量就是移动到当前节点或需要移动到别的节点的量：
    - 如果叶子节点值为0，说明还需要1枚硬币，过载量就是1
    - 如果叶子节点值为4，说明会有3个硬币会被移动，过载量就是3
    - 所以每个叶子节点的过载量`Math.abs(node.val - 1)`
- 整个子树的过载量就是`整棵树的节点值-整棵子树的应有的节点值`的值
    - 所以每个节点的过载量就是`node.val + dfs(node.left) + dfs(node.right) - 1`
### 代码
```java
class Solution {
    private int ans = 0;
