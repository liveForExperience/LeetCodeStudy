...
```
# [LeetCode_421_数组中两个数的最大异或值](https://leetcode-cn.com/problems/maximum-xor-of-two-numbers-in-an-array/)
## 解法
### 思路
字典树
- 遍历nums生成一个二叉字典树
- 遍历nums，根据当前num，在字典树中找到异或值最大的值，也就是路径中如果有和当前位上值不同的值，就优先选这个节点下探，否则选另一个
- 遍历完后，取最大的那个异或结果
### 代码
```java
class Solution {
